...
//-----------------------------------------------------------------------------
client_message_buffer_c::buffer_error_e client_message_buffer_c::write( const client_message_t& m, bool write_plan ) {
  assert( _open );
  ...
  memcpy( &_buffer->control, &m.control, sizeof(struct control_t) );
  if( write_plan )
    memcpy( &_buffer->plan, &m.plan, sizeof(struct plan_t) );
...
//-----------------------------------------------------------------------------
client_message_buffer_c::buffer_error_e client_message_buffer_c::read( client_message_t& m, bool read_plan ) {
  assert( _open );
  ...
  memcpy( &m.control, &_buffer->control, sizeof(struct control_t) );
  if( read_plan ) 
    memcpy( &m.plan, &_buffer->plan, sizeof(struct plan_t) );
