...
void scheduler_c::process_wakeups( const thread_p& caller, thread_heap_c& runqueue, thread_heap_c& waitqueue, log_c* info = NULL ) {
  ...
  if( runqueue.empty() ) {
    ...
    // TODO : everything that needs to wake can be compared by whiling around
    // heap top and comparing to the temporal progress.  This reduces the
    // time complexity by not having to iterate over the whole heap.
...
/*
  if( next_thread->type() == thread_c::TIMESINK ) {
    timesink_p sink = boost::dynamic_pointer_cast<timesink_c>( next_thread );
    printf( "next_thread run_queue[%d], block_queue[%d]\n", sink->run_queue.size(), sink->block_queue.size() );
  }
*/
...
/// @param processor the processor to bind the process to.
/// @param program the name of the program to execute.
/// @param arg argument supplied to client process.
/// @param suppress_console_io argument flags whether the child process is 
///        allowed to read and write information to the console.
/// @return indicator of operation success or specified error.
scheduler_c::error_e scheduler_c::create( const osthread_p& thread, int priority_offset, cpu_id_t cpu, const char* program, const char* arg, bool suppress_console_io = false ) {
  ...
  if( thread->pid > 0 ) {
    ...
    // TODO : remove these printf's in the parent process space.
    if( cpu_c::bind( thread->pid, cpu ) != cpu_c::ERROR_NONE )
      printf( "failed to bind osthread\n" );

    // set the scheduling policy and priority
    if( set_realtime_policy( thread->pid, priority, priority_offset ) != scheduler_c::ERROR_NONE )
      printf( "failed to set realtime policy on osthread\n" );

    return ERROR_NONE;
  }

  printf( "client program %s is starting.\n", program );\

  if( suppress_console_io ) {
    printf( "NOTICE: client I/O from %s will be supressed.  I/O using stdout, stdin, and stderr will be forwarded to /dev/null.\n", program );
    ...
    printf( "WARNING: client I/O from %s has NOT been suppressed.  This will significantly affect performance.  Scheduling behavior and temporal progress can NOT be guaranteed.\n", program );
  }
  ...
  // bind the child to the cpu  
  if( cpu_c::bind( pid, cpu ) != cpu_c::ERROR_NONE ) {
    printf( "failed to bind client\n" );
    _exit( 1 );     // kill the child process
  }

  // set the scheduling policy and priority
  if( set_realtime_policy( pid, priority, priority_offset ) != scheduler_c::ERROR_NONE ) {
    printf( "failed to set realtime policy on client\n" );
    _exit( 1 );     // kill the child process
  }
