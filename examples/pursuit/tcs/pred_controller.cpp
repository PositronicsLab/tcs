...
//-----------------------------------------------------------------------------
static unsigned current_plan_idx;
static double current_plan_ctl_start;
...
void compute_command( void ) {
  ...
  predator->time = (double)t;
  predator->dtime = (double)dt;
  prey->time = (double)t;
  prey->dtime = (double)dt;

  printf( "predator: time[%f], dtime[%f]\n", t, dt );
  ...
  Ravelin::SForced drag_force = Ravelin::Pose3d::transform(GLOBAL, ship_c::drag_force(v , P ));
  ...
  for (unsigned i=0; i< 6; i++)
    pred_control[i] += drag_force[i];

  // EMD 5/19/14 -- zero torque
  pred_control[3] = pred_control[4] = pred_control[5];
}
...
void request_state( void ) {
  ...
  if( server_note.source == notification_t::SERVER ) {
    if( server_note.type == notification_t::READ ) {
      ...
      predator->adversary->time = msg.header.t;
      predator->adversary->dtime = msg.header.dt;
      prey->adversary->time = msg.header.t;
      prey->adversary->dtime = msg.header.dt;
      ...
      for( unsigned i = 0; i < 6; i++ )
        pred_planned_control[i] = msg.control.u[i];
        //predator->planned_control[i] = msg.control.u[i];

      //printf( "pred planned_control[%f,%f,%f,%f,%f,%f]\n", predator->planned_control[0], predator->planned_control[1], predator->planned_control[2], predator->planned_control[3], predator->planned_control[4], predator->planned_control[5] );
    }
  }
...
bool publish_command( void ) {
  ...
  msg.control.ts = ts;
  for( unsigned i = 0; i < 6; i++ )
    msg.control.u[i] = pred_control[i];
  msgbuffer.write( msg );
